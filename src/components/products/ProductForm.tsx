...
export default function ProductForm({ product, onClose, onSuccess, inline = false }: ProductFormProps) {
  ...
  // Backend returns image=URL string, images=URL[]. Map to Media-like for display
  const urlToMedia = (url: string, id = ''): Media => ({
    id: id || `url-${url}`,
  ...
  const [featuredImage, setFeaturedImage] = useState<Media | null>(() => {
    if (product?.featured_image?.file_url) return product.featured_image as Media
    if (product?.image && typeof product.image === 'string') return urlToMedia(product.image, 'featured')
    return null
  })
  const [additionalImages, setAdditionalImages] = useState<Media[]>(() => {
    if (!product?.images?.length) return []
    return product.images.map((item: any) => {
      if (typeof item === 'string') return urlToMedia(item)
      if (item?.media?.file_url) return item.media as Media
      if (item?.file_url) return urlToMedia(item.file_url, item.id)
      return null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!product && !featuredImage?.file_url) newErrors.image = 'Main image is required'
      ...
      const payload = {
        ...formData,
        // Backend expects image=URL (required), images=array of URLs
        image: featuredImage?.file_url || (featuredImage as any)?.url || featuredImage?.thumbnail_url || '',
        category: formData.category_id,
        stock_quantity: formData.quantity,
        images: additionalImages.map(m => m.file_url || (m as any)?.url || m.thumbnail_url).filter(Boolean)
      }
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="flex flex-wrap items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <span className="text-xs text-text-muted">
                  {formData.status === 'draft' && 'Product is hidden. Set to Active to show on store.'}
                  {formData.status === 'active' && 'Product is live on the store.'}
                  {formData.status === 'archived' && 'Product is archived.'}
                </span>
              </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  ...
                  <div className="aspect-square bg-gray-50 rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center relative group hover:border-forest-primary/50 transition-colors">
                    ...
                    <div className="text-center p-4">
                      {uploadingImage ? (
                        <Loader2 className="w-6 h-6 animate-spin text-forest-primary mx-auto" />
                      ) : (
                        <>
                          <Plus className="w-6 h-6 text-gray-300 mx-auto group-hover:text-forest-primary transition-colors" />
                          <span className="text-[10px] font-bold text-text-muted uppercase mt-1 block group-hover:text-forest-primary">Add More</span>
                        </>
                      )}
                    </div>
                  </div>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50 flex flex-col gap-4 sticky bottom-0 z-10">
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              ...
              className="btn btn-primary px-10 flex items-center gap-2 font-bold shadow-lg shadow-forest-primary/20"
            >
              {loading ? (

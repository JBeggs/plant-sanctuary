...
function normaliseHero(row: ApiHero): PageHero | null {
  ...
  return {
    ...
    imageUrl: rawUrl ? ensureAbsoluteImageUrl(rawUrl) : null,
...
export default function BrandingClient() {
  ...
  const fetchHeroes = useCallback(async () => {
    ...
    try {
      ...
      setHeroes(rows.map(normaliseHero).filter((row): row is PageHero => row !== null))
    } catch (err) {
    ...
    <div className="min-h-screen bg-forest-background pb-20">
      ...
      <div className="container-wide py-8 space-y-6">
        <p className="text-text-muted text-sm max-w-2xl">
          Upload a banner image for any page below. Toggle the hero on to replace that page&apos;s
          default header with your image; toggle off to use the standard layout.
        </p>
